...
uses unConstantes, unDM, unSelecionaRegistroEstado, unPesquisandoCidade,
  unVariaveis, unSelecionaRegistroCidade, Math, unDicas, unPrincipal;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          Self.FoEstadoAux.nome := DM.zQryEstadoNOME.AsString;
        end;
      end;
    end;
  end;
...
begin
  ...
  case pnVisib of
    ...
    OPC_NOVO:        begin
                       plsEdCodigo.Text := IntToStr(FoFuncoes.RetornarProximoCodigo(TABLE_CIDADE,True));
    ...
    begin
      try
        ...
        FoFuncoes.AjustarPosicaoFormCadastroSelecaoRegistro(frmPrincipal,
         frmCadCidade,frmSelecionaRegistroEstado,pnlEstado,ToolBar);
        frmSelecionaRegistroEstado.passarParametro(PRM_DEFINIR_TOP_FORM_SELECAO_REGISTRO,2);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if(FoPesquisa.PodeAbrirTelaPesquisandoRegistro)then //04/04/2011
          begin
            FoPesquisa.Show;
            FoFuncoes.AjustarPosicaoFormCadastroPesquisa(frmPrincipal, frmCadCidade, FoPesquisa);            
          end;
        end;
      end;
    end;
...
//    AbreTelaSelecaoEstado(fnSELECIONAR_ESTADO_NOMRAL);
  //04/04/2011
  MostrarHintCampos(STRING_INDEFINIDO,'Nome do Estado');
end;
...
procedure TfrmCadCidade.plsEdCodigoEnter(Sender: TObject);
begin
  inherited;
  ...
  if(Assigned(Self.FoPesquisa))then
    Self.FoPesquisa.Hide;
  //04/04/2011
  MostrarHintCampos('Cidade pelo Código',STRING_INDEFINIDO);
end;

procedure TfrmCadCidade.plsEdNomeEnter(Sender: TObject);
begin
  inherited;
  ...
  if(Assigned(Self.FoPesquisa))then
    Self.FoPesquisa.Hide;
  //04/04/2011
  MostrarHintCampos('Cidade pelo Nome','Nome da Cidade');
end;

procedure TfrmCadCidade.plsEdEstadoEnter(Sender: TObject);
begin
  inherited;
...
procedure TfrmCadCidade.lblDicasClick(Sender: TObject);
begin
  inherited;
  FoFuncoes.CriarTelaDicas(FORM_CAD_CIDADE);
end;
