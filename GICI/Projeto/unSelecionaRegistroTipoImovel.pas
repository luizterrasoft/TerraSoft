(*
tela está ok para implementação dos métodos/padrões
*)
...
uses unConstantes, unCadImoveis, unCadImovelInteresse, unCadTipoImovel,unPesquisarAgenciamentos,
...
begin
  ...
  begin
    ...
    if(MessageDlg('Selecionar o tipo de imóvel que foi cadastrado?',mtConfirmation,[mbYes,mbNo],0)=mrYes)then
    begin
...
begin
  ...
  begin
    if(FiTelaChamou = FORM_CAD_IMOVEL)then //TfrmCadImoveis
    begin
      frmCadImoveis.FoTipoImovel.codigo := FiCodigo;
      frmCadImoveis.FoTipoImovel.nome := FsNome;
    end
    else if(FiTelaChamou = FORM_CAD_IMOVEL_INTERESSE)then //TfrmCadImovelInteresse
    begin
    ...
    begin
      frmPesquisarAgenciamentos.FoTipoImovel.codigo := FiCodigo;
      frmPesquisarAgenciamentos.FoTipoImovel.nome := FsNome;
    end
    else if(FiTelaChamou = FORM_PESQUISAR_IMOVEL_INTERESSE)then //TfrmPesquisarImoveisInteresse
    begin
      frmPesquisarImoveisInteresse.FoTipoImovel.codigo := FiCodigo;
      frmPesquisarImoveisInteresse.FoTipoImovel.nome := FsNome;
    end
    else if(FiTelaChamou = FORM_PESQUISAR_VENDA)then //TfrmPesquisarVendas
    begin
      frmPesquisarVendas.FoTipoImovel.codigo := FiCodigo;
      frmPesquisarVendas.FoTipoImovel.nome := FsNome;
    end;
  end;
...
begin
  ...
  if(FiTelaChamou = FORM_CAD_IMOVEL_INTERESSE)then //TfrmCadImovelInteresse
    frmCadImovelInteresse.passarParametro(PRM_SELECIONAR_TODOS_REGISTROS_CONJUNTO_DADOS, vParametros)

  else if(FiTelaChamou = FORM_CAD_IMOVEL)then //TfrmCadImoveis
