...
uses unCadCliente, unConstantes, unCadPessoaFisica, unCadImoveis, unCadRua,
  ...
  unPesquisarAgenciamentos, unPesquisarVendas;
...
begin
  ...
  begin
    ...
    begin
      frmPesquisarAgenciamentos.FoRua.codigo := FiCodigo;
      frmPesquisarAgenciamentos.FoRua.nome := FsNome;
      frmPesquisarAgenciamentos.FoRua.cep := FsCEP;
    end
    else if(FiTelaChamou = FORM_PESQUISAR_VENDA)then //TfrmPesquisarVendas
    begin
      frmPesquisarVendas.FoRua.codigo := FiCodigo;
      frmPesquisarVendas.FoRua.nome := FsNome;
      frmPesquisarVendas.FoRua.cep := FsCEP;
    end;
  end;
...
procedure TfrmSelecionaRegistroRua.FormShow(Sender: TObject);
begin
  inherited;
  FoFuncoes.focarComponente(plsEdValorPesquisar);
end;
