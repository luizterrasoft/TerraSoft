...
uses
  ...
  DBClient, StdCtrls, plsComboBox, Buttons, ExtCtrls, plsMemo, unFuncoes,
  MoneyEdit;
...
uses unDM, unConstantes, unCadImoveis, unCadImovelInteresse,
  unPesquisarAgenciamentos, unPesquisarImoveisInteresse;
...
begin
  ...
  else if(FoReceberCarac = nRECEBER_CARAC_PESQUISAR_AGENCIAMENTO_DESCRICAO)then
    frmPesquisarAgenciamentos.plsMmDescricao.Text := FsCaracateristicas
  else if(FoReceberCarac = nRECEBER_CARAC_PESQUISAR_IMOVEL_INTERESSE_CARACTERISTICA)then
    frmPesquisarImoveisInteresse.plsMmCaracteristica.Text := FsCaracateristicas;
end;
...
procedure TfrmSelecionarCaracImovel.FormDestroy(Sender: TObject);
begin
