unit unSelecionaRegistroRegiaoLocalidade;
...
type
  TfrmSelecionaRegistroRegiaoLocalidade = class(TfrmSelecionaRegistro)
...
var
  frmSelecionaRegistroRegiaoLocalidade: TfrmSelecionaRegistroRegiaoLocalidade;

implementation

uses unCadImovelInteresse, unConstantes, unCadRegiaoLocalidade,
  unPesquisarImoveisInteresse;

{$R *.dfm}

{ TfrmSelecionaRegistroRegiaoLocalidade }

{***** Minhas Procedures - inicio *****}

procedure TfrmSelecionaRegistroRegiaoLocalidade.passarParametro(pTipo: String; pValores: OleVariant);
var
  sCodRegiaoLocNovo: String;
begin
  inherited;
  ...
  begin
    ...
    if(MessageDlg('Selecionar a Região Localidade que foi cadastrada?',mtConfirmation,[mbYes,mbNo],0)=mrYes)then
    begin
      sCodRegiaoLocNovo := pValores;
      cdsSelecionarRegistro.Locate('CODIGO', sCodRegiaoLocNovo, []);
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.filtrarPesquisa();
begin
  ...
  begin
    if(FbMostraMensagemNaoEncontrouRegistro)then
      MessageDlg('Nenhuma região localidade encontrada.',mtInformation,[mbOK],0);
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.selecionarTodosRegistros();
begin
  ...
  begin
    ...
    zQrySelecionarRegistro.SQL.Add('SELECT r.codigo, r.nome,  CAST(r.codigo AS VARCHAR(10)) AS cc_codigo ');
    zQrySelecionarRegistro.SQL.Add('FROM regiao_localidade r ');
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.selecionouRegistro();
begin
  ...
  begin
    ...
    begin
      frmCadImovelInteresse.FoRegiaoLocalidade.codigo := FiCodigo;
      frmCadImovelInteresse.FoRegiaoLocalidade.nome := FsNome;
    end
    else if(Self.FiTelaChamou = FORM_PESQUISAR_IMOVEL_INTERESSE)then //TfrmPesquisarImoveisInteresse
    begin
      frmPesquisarImoveisInteresse.FoRegiaoLocalidade.codigo := FiCodigo;
      frmPesquisarImoveisInteresse.FoRegiaoLocalidade.nome := FsNome;
    end;
  end;
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.configurarCaracteresAceitosPesquisa();
begin
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.inserirNovoRegistro();
begin
  try
    ...
    frmCadRegiaoLocalidade.passarParametro(PRM_INSERE_REGISTRO_AO_ESTAR_SELECIONANDO, FORM_SELECIONA_REGISTRO_REGIAO_LOCALIDADE);
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.ConfigurarCampoPesquisa();
begin
  plsEdValorPesquisar.Clear;
  FoFuncoes.focarComponente(plsEdValorPesquisar);
...
//seleciona todos os registros do conjunto de dados da tela pai
procedure TfrmSelecionaRegistroRegiaoLocalidade.SelecionarTodosRegistrosConjuntoDadosTelaPai();
var
  vParametros: Variant;
begin
  vParametros := VarArrayCreate([0,0],varVariant);
  vParametros[0] := CONJUNTO_DADOS_REGIAO_LOCALIDADE;
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.FormCreate(
  Sender: TObject);
begin
  inherited;
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.plsCbBxCampoPesquisarChange(
  Sender: TObject);
begin
  inherited;
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.cdsSelecionarRegistroAfterScroll(
  DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmSelecionaRegistroRegiaoLocalidade.plsEdValorPesquisarChange(
  Sender: TObject);
begin
  inherited;
  inherited;
