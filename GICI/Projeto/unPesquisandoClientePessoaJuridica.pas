...
*)
unit unPesquisandoClientePessoaJuridica;
...
type
  TfrmPesquisandoClientePessoaJuridica = class(TfrmPesquisandoRegistro)
...
var
  frmPesquisandoClientePessoaJuridica: TfrmPesquisandoClientePessoaJuridica;

implementation

uses unCadClientePessoaJuridica, unConstantes;

{$R *.dfm}

{ TfrmPesquisandoClientePessoaJuridica }

(* procedimentos de controle *)

procedure TfrmPesquisandoClientePessoaJuridica.passarParametro(pTipo:String; pValores:OleVariant);
begin
...
procedure TfrmPesquisandoClientePessoaJuridica.filtrarPesquisa();
var
  sVlrPesquisa: String;
begin
...
procedure TfrmPesquisandoClientePessoaJuridica.selecionouRegistro();
begin
  if(cdsPesquisando.Active)then
  begin
    if(Self.FoNomeFormChamou is TfrmCadClientePessoaJuridica)then
    begin
      (Self.FoNomeFormChamou as TfrmCadClientePessoaJuridica).passarParametro(PRM_PESQUISOU,null);
      (Self.FoNomeFormChamou as TfrmCadClientePessoaJuridica).realizouPesquisa(cdsPesquisandoCODIGO.AsInteger);
    end;
    Self.Hide;
  end;
end;

procedure TfrmPesquisandoClientePessoaJuridica.configurarCaracteresAceitosPesquisa();
begin
...
procedure TfrmPesquisandoClientePessoaJuridica.selecionarTodosRegistros();
var
  sql: String;
begin
  ...
  sql := sql + 'FROM cliente_pessoa_juridica p ';
...
procedure TfrmPesquisandoClientePessoaJuridica.popularClientDataSetPesquisa(poDados: TClientDataSet);
begin

end;

procedure TfrmPesquisandoClientePessoaJuridica.atualizarPesquisando(psCampoPesquisa: string;
...
procedure TfrmPesquisandoClientePessoaJuridica.plsCbBxCampoPesquisarChange(
  Sender: TObject);
begin
  inherited;
...
procedure TfrmPesquisandoClientePessoaJuridica.plsEdValorPesquisaChange(
  Sender: TObject);
begin
  inherited;
