...
uses
  ...
  type TConjunto = class
    ...
    published
      ...
      procedure DeletarRegistroNaoDefinido();
...
begin
  FcdsDados.Close;
  FdspDataSetProvider.DataSet := nil;
  FzQuery.Close;
  FzQuery.SQL.Clear;
...
begin
  ...
  FcdsDados.Filter := GetFiltro; //FsFiltro;
...
procedure TConjunto.DeletarRegistroNaoDefinido;
begin
  FcdsDados.First;
  if(FcdsDados.Locate('codigo',REGISTRO_NULO,[]))then
  begin
    FcdsDados.Delete;
    FcdsDados.First;
  end;
