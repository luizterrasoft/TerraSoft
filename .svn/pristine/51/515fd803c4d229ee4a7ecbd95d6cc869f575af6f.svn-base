...
uses
  ...
  ComCtrls, ToolWin, plsMemo, unObjMeioCaptacao, ExtCtrls;
...
uses unDM, unPesquisandoMeioCaptacao, unConstantes, unVariaveis,
  unSelecionaRegistroMeioCaptacao, unPrincipal;
...
begin
  ...
  case pnVisib of
    ...
    OPC_NOVO:        begin
                       plsEdCodigo.Text := IntToStr(FoFuncoes.RetornarProximoCodigo(TABLE_MEIO_CAPTACAO,True));    
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          FoPesquisa.Show;
          FoFuncoes.AjustarPosicaoFormCadastroPesquisa(frmPrincipal, frmCadMeioCaptacao, FoPesquisa);
        end;
      end;
    end;
  end;
...
procedure TfrmCadMeioCaptacao.plsEdCodigoEnter(Sender: TObject);
begin
  inherited;
  MostrarHintCampos('Meio de Captação pelo Código',STRING_INDEFINIDO);
end;

procedure TfrmCadMeioCaptacao.plsEdCodigoKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  MostrarHintCampos('Meio de Captação pelo Código',STRING_INDEFINIDO);
...
procedure TfrmCadMeioCaptacao.plsEdNomeEnter(Sender: TObject);
begin
  inherited;
  MostrarHintCampos('Meio de Captação pelo Nome', 'Nome do Meio de Captação');
end;

procedure TfrmCadMeioCaptacao.plsEdNomeKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  MostrarHintCampos('Meio de Captação pelo Nome', 'Nome do Meio de Captação');
...
procedure TfrmCadMeioCaptacao.plsMmDescricaoEnter(Sender: TObject);
begin
  inherited;
  MostrarHintCampos(STRING_INDEFINIDO, 'Descrição do Meio de Captação', False);
end;

procedure TfrmCadMeioCaptacao.plsMmDescricaoKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  MostrarHintCampos(STRING_INDEFINIDO, 'Descrição do Meio de Captação', False);
...
procedure TfrmCadMeioCaptacao.lblDicasClick(Sender: TObject);
begin
  inherited;
  FoFuncoes.CriarTelaDicas(FORM_CAD_MEIO_CAPTACAO);
end;
