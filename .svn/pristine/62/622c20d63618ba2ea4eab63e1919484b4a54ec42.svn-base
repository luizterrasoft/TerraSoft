...
uses unConstantes, unPesquisandoBairro, unDM, unSelecionaRegistroCidade,
  unVariaveis, unSelecionaRegistroBairro, unDicas, unPrincipal;
...
begin
  ...
  FoPesquisa.iniciarTela(frmCadBairro);
...
begin
  ...
  //verifica se bairro não está associado a um cliente pessoa física
  ...
  //verifica se bairro não está associado a um cliente pessoa jurídica
  if(bPodeDeletar)then
  begin
  ...
  //verifica se bairro não está associado a um  agenciamento pessoa_fisica
  if(bPodeDeletar)then
  begin
    ...
    begin
      MessageDlg('Bairro não pode ser deletado, pois existe um agenciador deste bairro.',
        mtInformation, [mbOK], 0);
      bPodeDeletar := False;
    end;
  end;

  //verifica se cidade não está associado a um agenciamento pessoa_juridica
  if(bPodeDeletar)then
  begin
...
begin
  ...
  case pnVisib of
    ...
    OPC_NOVO:        begin
                       plsEdCodigo.Text := IntToStr(FoFuncoes.RetornarProximoCodigo(TABLE_BAIRRO,True));    
    ...
    begin
      try
        ...
        FoFuncoes.AjustarPosicaoFormCadastroSelecaoRegistro(frmPrincipal,
         frmCadBairro,frmSelecionaRegistroCidade,pnlCidade,ToolBar);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          FoPesquisa.Show;
          FoFuncoes.AjustarPosicaoFormCadastroPesquisa(frmPrincipal, frmCadBairro, FoPesquisa);
        end;
      end;
    end;
  end;
...
procedure TfrmCadBairro.plsEdNomeEnter(Sender: TObject);
begin
  inherited;
  MostrarHintCampos('Bairro pelo Nome','Nome do Bairro');
end;

procedure TfrmCadBairro.plsEdCidadeEnter(Sender: TObject);
begin
  inherited;
  MostrarHintCampos(STRING_INDEFINIDO,'Nome da Cidade');
end;

procedure TfrmCadBairro.plsEdCodigoEnter(Sender: TObject);
begin
  inherited;
  MostrarHintCampos('Bairro pelo Código',STRING_INDEFINIDO);
end;

procedure TfrmCadBairro.lblDicasClick(Sender: TObject);
begin
  inherited;
  FoFuncoes.CriarTelaDicas(FORM_CAD_BAIRRO);
end;

procedure TfrmCadBairro.plsEdCodigoKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  MostrarHintCampos('Bairro pelo Código',STRING_INDEFINIDO);
...
procedure TfrmCadBairro.plsEdNomeKeyPress(Sender: TObject; var Key: Char);
begin
  inherited;
  MostrarHintCampos('Bairro pelo Nome','Nome do Bairro');
end;
