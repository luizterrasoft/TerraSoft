...
*)
unit unPesquisandoPessoaFisica;
...
uses
  ...
  ExtCtrls, Grids, DBGrids, plsComboBox, ZAbstractRODataset,
  ZAbstractDataset, ZDataset, Provider, unFuncoes;

type
  TfrmPesquisandoPessoaFisica = class(TfrmPesquisandoRegistro)
    ...
    zQryPesquisandoDATA_NASCIMENTO: TDateTimeField;
    ...
    cdsPesquisandoDATA_NASCIMENTO: TDateTimeField;
...
var
  frmPesquisandoPessoaFisica: TfrmPesquisandoPessoaFisica;

implementation

uses unConstantes, unDM, unCadPessoaFisica;

{$R *.dfm}

{ TfrmPesquisandoPessoaFisica }

(* procedimentos de controle *)

procedure TfrmPesquisandoPessoaFisica.passarParametro(pTipo:String; pValores:OleVariant);
begin
...
procedure TfrmPesquisandoPessoaFisica.filtrarPesquisa();
var
  sVlrPesquisa: String;
begin
...
procedure TfrmPesquisandoPessoaFisica.selecionouRegistro();
begin
  if(cdsPesquisando.Active)then
  begin
    if(Self.FoNomeFormChamou is TfrmCadPessoaFisica)then
    begin
      (Self.FoNomeFormChamou as TfrmCadPessoaFisica).passarParametro(PRM_PESQUISOU,null);
      (Self.FoNomeFormChamou as TfrmCadPessoaFisica).realizouPesquisa(cdsPesquisandoCODIGO.AsInteger);
    end;
    Self.Hide;
  end;
end;

procedure TfrmPesquisandoPessoaFisica.configurarCaracteresAceitosPesquisa();
begin
...
procedure TfrmPesquisandoPessoaFisica.selecionarTodosRegistros();
var
  sql: String;
begin
  cdsPesquisando.IndexName := '';
  sql := 'SELECT p.codigo,p.nome,p.email,p.cpf,p.identidade,p.data_nascimento,';
  sql := sql + 'p.telefone_residencial,p.telefone_comercial,p.telefone_celular,';
  sql := sql + 'p.cod_fk_estado,p.cod_fk_cidade,e.sigla as sigla_estado,';
  sql := sql + 'c.nome as nome_cidade,CAST(p.codigo AS VARCHAR(10))AS cc_codigo ';
...
procedure TfrmPesquisandoPessoaFisica.popularClientDataSetPesquisa(poDados: TClientDataSet);
begin

end;

procedure TfrmPesquisandoPessoaFisica.atualizarPesquisando(psCampoPesquisa: string;
...
procedure TfrmPesquisandoPessoaFisica.plsCbBxCampoPesquisarChange(
  Sender: TObject);
begin
  inherited;
...
procedure TfrmPesquisandoPessoaFisica.plsEdValorPesquisaChange(
  Sender: TObject);
begin
  inherited;
