unit unPesquisandoRegiaoLocalidade;
...
type
  TfrmPesquisandoRegiaoLocalidade = class(TfrmPesquisandoRegistro)
...
var
  frmPesquisandoRegiaoLocalidade: TfrmPesquisandoRegiaoLocalidade;

implementation

uses unCadRegiaoLocalidade, unConstantes;

{$R *.dfm}

{ TfrmPesquisandoRegiaoLocalidade }

(* procedimentos de controle *)

procedure TfrmPesquisandoRegiaoLocalidade.passarParametro(pTipo:String; pValores:OleVariant);
begin
  inherited;
...
procedure TfrmPesquisandoRegiaoLocalidade.selecionarTodosRegistros();
begin
end;

procedure TfrmPesquisandoRegiaoLocalidade.popularClientDataSetPesquisa(poDados: TClientDataSet);
begin
...
procedure TfrmPesquisandoRegiaoLocalidade.OrdenarGrideRegistrosInciarTela;
var
  sIndexName: String;
...
procedure TfrmPesquisandoRegiaoLocalidade.filtrarPesquisa();
begin
...
procedure TfrmPesquisandoRegiaoLocalidade.selecionouRegistro();
begin
  if(cdsPesquisando.Active)then
  begin
    if(Self.FoNomeFormChamou is TfrmCadRegiaoLocalidade)then
    begin
      (Self.FoNomeFormChamou as TfrmCadRegiaoLocalidade).passarParametro(PRM_PESQUISOU,null);
      (Self.FoNomeFormChamou as TfrmCadRegiaoLocalidade).realizouPesquisa(cdsPesquisandoCODIGO.AsInteger);
    end;
    Self.Hide;
  end;
end;

procedure TfrmPesquisandoRegiaoLocalidade.configurarCaracteresAceitosPesquisa();
begin
...
procedure TfrmPesquisandoRegiaoLocalidade.FormatarValores();
begin
end;
