...
uses
  ...
  StdCtrls, plsEdit, unObjProfissao, ExtCtrls;
...
uses unConstantes, unPesquisandoProfissao, unDM, unVariaveis,
  unSelecionaRegistroProfissao, unPrincipal;
...
begin
  ...
  FoPesquisa.iniciarTela(frmCadProfissao);
...
begin
  ...
  case pnVisib of
    ...
    OPC_NOVO:        begin
                       plsEdCodigo.Text := IntToStr(FoFuncoes.RetornarProximoCodigo(TABLE_PROFISSAO,True));    
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FoFuncoes.AjustarPosicaoFormCadastroPesquisa(frmPrincipal, frmCadProfissao, FoPesquisa);
        end;
      end;
    end;
  end;
...
procedure TfrmCadProfissao.plsEdCodigoEnter(Sender: TObject);
begin
  inherited;
  MostrarHintCampos('Profissão pelo Código',STRING_INDEFINIDO);
end;

procedure TfrmCadProfissao.plsEdCodigoKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  MostrarHintCampos('Profissão pelo Código',STRING_INDEFINIDO);
...
procedure TfrmCadProfissao.plsEdNomeEnter(Sender: TObject);
begin
  inherited;
  MostrarHintCampos('Profissão pelo Nome', 'Nome da Profissão');
end;

procedure TfrmCadProfissao.plsEdNomeKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  MostrarHintCampos('Profissão pelo Nome', 'Nome da Profissão');
...
procedure TfrmCadProfissao.lblDicasClick(Sender: TObject);
begin
  inherited;
  FoFuncoes.CriarTelaDicas(FORM_CAD_PROFISSAO);
end;
