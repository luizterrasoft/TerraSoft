object frmAgenciamentosParaImovelInteresse: TfrmAgenciamentosParaImovelInteresse
  Left = 75
  Top = 36
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'AGENCIAMENTOS PARA IM'#211'VEL DE INTERESSE'
  ClientHeight = 588
  ClientWidth = 994
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnKeyPress = FormKeyPress
  PixelsPerInch = 96
  TextHeight = 13
  object StatusBar: TStatusBar
    Left = 0
    Top = 569
    Width = 994
    Height = 19
    Panels = <
      item
        Width = 50
      end>
  end
  object grpBxImovInteresse: TGroupBox
    Left = 0
    Top = 0
    Width = 994
    Height = 177
    Align = alTop
    Caption = 'Im'#243'vel de Interesse '
    TabOrder = 1
    object lblInfValorImovel: TLabel
      Left = 510
      Top = 20
      Width = 61
      Height = 13
      Caption = 'Valor Im'#243'vel:'
    end
    object lblInfCodigoImovInteresse: TLabel
      Left = 8
      Top = 20
      Width = 39
      Height = 13
      Caption = 'C'#243'digo: '
    end
    object lblCodigoImovInteresse: TLabel
      Left = 104
      Top = 20
      Width = 110
      Height = 13
      AutoSize = False
      Caption = 'lblCodigoImovInteresse'
    end
    object lblInfAtivo: TLabel
      Left = 215
      Top = 20
      Width = 30
      Height = 13
      Caption = 'Ativo: '
    end
    object lblAtivo: TLabel
      Left = 246
      Top = 20
      Width = 72
      Height = 13
      AutoSize = False
      Caption = 'lblAtivo'
    end
    object lblValorImovel: TLabel
      Left = 600
      Top = 20
      Width = 65
      Height = 13
      Caption = 'lblValorImovel'
    end
    object lblInfFinalidadeImovel: TLabel
      Left = 215
      Top = 50
      Width = 103
      Height = 13
      Caption = 'Finalidade do Im'#243'vel: '
    end
    object lblFinalidadeImovel: TLabel
      Left = 320
      Top = 50
      Width = 185
      Height = 13
      AutoSize = False
      Caption = 'lblFinalidadeImovel'
    end
    object lblInfImovelDesejavel: TLabel
      Left = 510
      Top = 50
      Width = 87
      Height = 13
      Caption = 'Im'#243'vel Desej'#225'vel: '
    end
    object lblImovelDesejavel: TLabel
      Left = 600
      Top = 50
      Width = 88
      Height = 13
      Caption = 'lblImovelDesejavel'
    end
    object lblInfCondicaoPgto: TLabel
      Left = 320
      Top = 20
      Width = 73
      Height = 13
      Caption = 'Condi'#231#227'o Pgto:'
    end
    object lblCondicaoPgto: TLabel
      Left = 398
      Top = 20
      Width = 109
      Height = 13
      AutoSize = False
      Caption = 'lblCondicaoPgto'
    end
    object lblNomePessoa: TLabel
      Left = 215
      Top = 80
      Width = 765
      Height = 13
      AutoSize = False
      Caption = 'lblNomePessoa'
    end
    object lblInfTipoImovel: TLabel
      Left = 8
      Top = 50
      Width = 58
      Height = 13
      Caption = 'Tipo Im'#243'vel:'
    end
    object lblInfTipoPessoa: TLabel
      Left = 8
      Top = 80
      Width = 65
      Height = 13
      Caption = 'Tipo Pessoa: '
    end
    object lblTipoImovel: TLabel
      Left = 104
      Top = 50
      Width = 110
      Height = 13
      AutoSize = False
      Caption = 'lblTipoImovel'
    end
    object lblTipoPessoa: TLabel
      Left = 104
      Top = 80
      Width = 110
      Height = 13
      AutoSize = False
      Caption = 'lblTipoPessoa'
    end
    object lblInfCaracteristicas: TLabel
      Left = 8
      Top = 140
      Width = 77
      Height = 13
      Caption = 'Caracter'#237'sticas: '
    end
    object lblCaracteristicas: TLabel
      Left = 104
      Top = 140
      Width = 873
      Height = 32
      AutoSize = False
      Caption = 'lblCaracteristicas'
      WordWrap = True
    end
    object lblInfRegiaoLocalidade: TLabel
      Left = 8
      Top = 110
      Width = 95
      Height = 13
      Caption = 'Regi'#227'o Localidade: '
    end
    object lblRegiaoLocalidade: TLabel
      Left = 104
      Top = 110
      Width = 876
      Height = 29
      AutoSize = False
      Caption = 'lblRegiaoLocalidade'
    end
  end
  object pnlDivisao: TPanel
    Left = 0
    Top = 177
    Width = 994
    Height = 16
    Align = alTop
    BevelOuter = bvNone
    TabOrder = 2
  end
  object pnlAgenciamentos: TPanel
    Left = 0
    Top = 193
    Width = 994
    Height = 376
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 3
    object lblDescricaoTela: TLabel
      Left = 8
      Top = 8
      Width = 559
      Height = 16
      Caption = 
        'Agenciamentos que podem atender as necessidades para o Im'#243'vel de' +
        ' Interesse'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clMaroon
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblAbrirExplicacaoRelevancia: TLabel
      Left = 914
      Top = 28
      Width = 58
      Height = 13
      Cursor = crHandPoint
      Caption = 'relev'#226'ncia ?'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clMaroon
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsUnderline]
      ParentFont = False
      OnClick = lblAbrirExplicacaoRelevanciaClick
    end
    object lblFiltrar: TLabel
      Left = 6
      Top = 28
      Width = 28
      Height = 13
      Caption = 'Filtrar:'
    end
    object lblTotalRegistros: TLabel
      Left = 890
      Top = 8
      Width = 96
      Height = 13
      Alignment = taRightJustify
      Caption = 'lblTotalRegistros'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clMaroon
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object dbGrdAgenciamento: TDBGrid
      Left = 3
      Top = 46
      Width = 987
      Height = 327
      Hint = 'Agenciamentos que podem satisfazer o im'#243'vel de interesse'
      DataSource = dsAgenciamentos
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      ReadOnly = True
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnDrawColumnCell = dbGrdAgenciamentoDrawColumnCell
      OnDblClick = dbGrdAgenciamentoDblClick
      OnTitleClick = dbGrdAgenciamentoTitleClick
      Columns = <
        item
          Expanded = False
          FieldName = 'CODIGO'
          Title.Caption = 'C'#243'digo'
          Width = 37
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IGUAL_TIPO_IMOVEL'
          Title.Caption = 'Tipo Im'#243'vel'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_TIPO_IMOVEL'
          Title.Caption = 'Tipo Im'#243'vel'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IGUAL_SITUACAO'
          Title.Caption = 'Situa'#231#227'o'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_SITUACAO'
          Title.Caption = 'Situa'#231#227'o'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IGUAL_CONDICAO_PAGAMENTO'
          Title.Caption = 'Cond. Pgto'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_CONDICAO_PAGAMENTO'
          Title.Caption = 'Cond. Pgto'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IGUAL_VALOR'
          Title.Caption = 'Valor'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_VALOR'
          Title.Caption = 'Valor'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IGUAL_CARACTERISTICAS_IMOVEL'
          Title.Caption = 'Carac. Im'#243'vel'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_CARACTERISTICAS_IMOVEL'
          Title.Caption = 'Carac. Im'#243'vel'
          Width = 80
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'IGUAL_REGIAO_LOCALIDADE'
          Title.Caption = 'Regi'#227'o Localidade'
          Width = 60
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'VALOR_REGIAO_LOCALIDADE'
          Title.Caption = 'Regi'#227'o Localidade'
          Width = 80
          Visible = True
        end
        item
          Alignment = taRightJustify
          Expanded = False
          FieldName = 'PESO'
          Title.Caption = 'Relev'#226'ncia'
          Width = 60
          Visible = True
        end>
    end
    object ckBxTipoImovelIgual: TCheckBox
      Left = 67
      Top = 28
      Width = 92
      Height = 17
      Caption = 'Tipo do Im'#243'vel'
      TabOrder = 1
      OnClick = ckBxTipoImovelIgualClick
    end
    object ckBxSituacaoIgual: TCheckBox
      Left = 212
      Top = 28
      Width = 64
      Height = 17
      Caption = 'Situa'#231#227'o'
      TabOrder = 2
      OnClick = ckBxSituacaoIgualClick
    end
    object ckBxCondPgtoIgual: TCheckBox
      Left = 340
      Top = 28
      Width = 125
      Height = 17
      Caption = 'Condi'#231#227'o Pagamento'
      TabOrder = 3
      OnClick = ckBxCondPgtoIgualClick
    end
    object ckBxValorIgual: TCheckBox
      Left = 507
      Top = 28
      Width = 73
      Height = 17
      Caption = 'Valor Igual'
      TabOrder = 4
      OnClick = ckBxValorIgualClick
    end
    object ckBxCaracImovelIgual: TCheckBox
      Left = 622
      Top = 28
      Width = 142
      Height = 17
      Caption = 'Caracter'#237'sticas do Im'#243'vel'
      TabOrder = 5
      OnClick = ckBxCaracImovelIgualClick
    end
    object ckBxRegiaoLocalidadeIgual: TCheckBox
      Left = 792
      Top = 28
      Width = 112
      Height = 17
      Caption = 'Regi'#227'o Localidade'
      TabOrder = 6
      OnClick = ckBxRegiaoLocalidadeIgualClick
    end
  end
  object dsAgenciamentos: TDataSource
    DataSet = cdsAgenciamentos
    Left = 48
    Top = 329
  end
  object cdsAgenciamentos: TClientDataSet
    Aggregates = <>
    FieldDefs = <>
    IndexDefs = <>
    Params = <>
    StoreDefs = True
    AfterScroll = cdsAgenciamentosAfterScroll
    Left = 20
    Top = 329
  end
  object ImageList: TImageList
    Left = 19
    Top = 357
    Bitmap = {
      
      000000000000}
  end
  object ApplicationEvents: TApplicationEvents
    OnHint = ApplicationEventsHint
    Left = 952
    Top = 12
  end
  object zQryImovInteresse: TZQuery
    Connection = DM.ZConnection
    SQL.Strings = (
      'SELECT i.codigo AS codImovInteresse,i.ativo,'
      '       i.finalidade_imovel,i.imovel_desejavel,'
      '       i.condicao_pagamento,i.valor_imovel,'
      '       i.caracteristicas AS caracteristicasImovInteresse,'
      '       i.tipo_pessoa,'
      '       CASE WHEN i.tipo_pessoa ='#39'F'#205'SICA'#39' THEN pf.nome'
      '            WHEN i.tipo_pessoa = '#39'JUR'#205'DICA'#39' THEN pj.nome'
      '            ELSE '#39'NENHUMA'#39' END AS NOME_TIPO_PESSOA,'
      '       t.nome AS nomeTipoImovel'
      ''
      'FROM imovel_de_interesse i'
      'JOIN tipo_imovel t ON i.cod_fk_tipo_imovel=t.codigo'
      
        'LEFT JOIN agenc_pessoa_fisica pf ON i.cod_fk_cliente_pessoa_fisi' +
        'ca = pf.codigo'
      
        'LEFT JOIN agenc_pessoa_juridica pj ON i.cod_fk_cliente_pessoa_ju' +
        'ridica = pj.codigo'
      'WHERE 1=2')
    Params = <>
    Left = 216
    Top = 112
    object zQryImovInteresseCODIMOVINTERESSE: TIntegerField
      FieldName = 'CODIMOVINTERESSE'
      Required = True
    end
    object zQryImovInteresseATIVO: TStringField
      FieldName = 'ATIVO'
      Size = 3
    end
    object zQryImovInteresseFINALIDADE_IMOVEL: TStringField
      FieldName = 'FINALIDADE_IMOVEL'
      Size = 10
    end
    object zQryImovInteresseIMOVEL_DESEJAVEL: TStringField
      FieldName = 'IMOVEL_DESEJAVEL'
    end
    object zQryImovInteresseCONDICAO_PAGAMENTO: TStringField
      FieldName = 'CONDICAO_PAGAMENTO'
    end
    object zQryImovInteresseVALOR_IMOVEL: TFloatField
      FieldName = 'VALOR_IMOVEL'
    end
    object zQryImovInteresseCARACTERISTICASIMOVINTERESSE: TStringField
      FieldName = 'CARACTERISTICASIMOVINTERESSE'
      Size = 500
    end
    object zQryImovInteresseTIPO_PESSOA: TStringField
      FieldName = 'TIPO_PESSOA'
      Size = 8
    end
    object zQryImovInteresseNOME_TIPO_PESSOA: TStringField
      FieldName = 'NOME_TIPO_PESSOA'
      ReadOnly = True
      Size = 100
    end
    object zQryImovInteresseNOMETIPOIMOVEL: TStringField
      FieldName = 'NOMETIPOIMOVEL'
      Required = True
      Size = 60
    end
  end
  object zQryRegiaoLocalidadeImovInteresse: TZQuery
    Connection = DM.ZConnection
    SQL.Strings = (
      
        'SELECT i.codigo,i.cod_fk_imovel_de_interesse,i.cod_fk_regiao_loc' +
        'alidade,'
      '       r.nome AS nomeRegiaoLocalidade'
      'FROM imov_interesse_regiao_local i'
      'JOIN regiao_localidade r ON i.cod_fk_regiao_localidade=r.codigo'
      'WHERE i.cod_fk_imovel_de_interesse=1'
      'AND 1=2')
    Params = <>
    Left = 244
    Top = 112
    object zQryRegiaoLocalidadeImovInteresseCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object zQryRegiaoLocalidadeImovInteresseCOD_FK_IMOVEL_DE_INTERESSE: TIntegerField
      FieldName = 'COD_FK_IMOVEL_DE_INTERESSE'
      Required = True
    end
    object zQryRegiaoLocalidadeImovInteresseCOD_FK_REGIAO_LOCALIDADE: TIntegerField
      FieldName = 'COD_FK_REGIAO_LOCALIDADE'
      Required = True
    end
    object zQryRegiaoLocalidadeImovInteresseNOMEREGIAOLOCALIDADE: TStringField
      FieldName = 'NOMEREGIAOLOCALIDADE'
      Required = True
      Size = 50
    end
  end
  object zQryCompletarAgenciamentos: TZQuery
    Connection = DM.ZConnection
    SQL.Strings = (
      'SELECT a.codigo,a.condicao_pagamento,a.valor_total,'
      '       i.situacao_desejavel,i.descricao,'
      '       t.nome AS nomeTipoImovel,'
      '       c.nome AS nomeCidade,'
      '       b.nome AS nomeBairro'
      'FROM agenciamentos a'
      'JOIN imovel i ON a.cod_fk_imovel=i.codigo'
      'JOIN tipo_imovel t ON i.cod_fk_tipo_imovel=t.codigo'
      'JOIN cidade c ON i.cod_fk_cidade=c.codigo'
      'JOIN bairro b ON i.cod_fk_bairro=b.codigo'
      'WHERE a.codigo = 1'
      'AND 1=2')
    Params = <>
    Left = 19
    Top = 385
    object zQryCompletarAgenciamentosCODIGO: TIntegerField
      FieldName = 'CODIGO'
      Required = True
    end
    object zQryCompletarAgenciamentosCONDICAO_PAGAMENTO: TStringField
      FieldName = 'CONDICAO_PAGAMENTO'
    end
    object zQryCompletarAgenciamentosVALOR_TOTAL: TFloatField
      FieldName = 'VALOR_TOTAL'
    end
    object zQryCompletarAgenciamentosSITUACAO_DESEJAVEL: TStringField
      FieldName = 'SITUACAO_DESEJAVEL'
    end
    object zQryCompletarAgenciamentosDESCRICAO: TStringField
      FieldName = 'DESCRICAO'
      Size = 500
    end
    object zQryCompletarAgenciamentosNOMETIPOIMOVEL: TStringField
      FieldName = 'NOMETIPOIMOVEL'
      Required = True
      Size = 60
    end
    object zQryCompletarAgenciamentosNOMECIDADE: TStringField
      FieldName = 'NOMECIDADE'
      Required = True
      Size = 100
    end
    object zQryCompletarAgenciamentosNOMEBAIRRO: TStringField
      FieldName = 'NOMEBAIRRO'
      Required = True
      Size = 100
    end
  end
  object cdsAgenciamentosFixo: TClientDataSet
    Aggregates = <>
    Params = <>
    Left = 20
    Top = 301
  end
end
