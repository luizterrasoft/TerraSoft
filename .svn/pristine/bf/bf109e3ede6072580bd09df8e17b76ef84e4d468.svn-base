...
begin
  ...
  if(psTela = 'ESTADO')then
    ...
    nRetorno := 3
  else if(psTela = 'REGIAO LOCALIDADE')then
    nRetorno := 4
  else if(psTela = 'CONSTRUTORA')then
    nRetorno := 5
  else if(psTela = 'PROFISSÃO')then
    nRetorno := 6
    ...
    nRetorno := 9
  else if(psTela = 'GRUPO DE CONFIGURAÇÃO DE BEM NEGOCIÁVEL')then
    nRetorno := 10
  else if(psTela = 'SUB-GRUPO DE CONFIGURAÇÃO DE BEM NEGOCIÁVEL')then
    nRetorno := 11
  else if(psTela = 'AGENCIADOR - PESSOA FÍSICA')then
    nRetorno := 12
  else if(psTela = 'AGENCIADOR - PESSOA JURÍDICA')then
    nRetorno := 13
  else if(psTela = 'CLIENTE - PESSOA FÍSICA')then
    nRetorno := 14
  else if(psTela = 'CLIENTE - PESSOA JURÍDICA')then
    nRetorno := 15
  else if(psTela = 'IMÓVEL')then
    nRetorno := 16
  else if(psTela = 'TIPO DE IMÓVEL')then
    nRetorno := 17
  else if(psTela = 'AGENCIAMENTO')then
    nRetorno := 18
  else if(psTela = 'IMÓVEL DE INTERESSE')then
    nRetorno := 19
  else if(psTela = 'VENDA')then
    nRetorno := 20;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            FoPesquisa.Show;
            FoFuncoes.AjustarPosicaoFormCadastroPesquisa(frmPrincipal, frmCadDica, FoPesquisa);
          end;
        end;
      end;
    end;
...
begin
  inherited;
  MostrarHintCampos(STRING_INDEFINIDO,'Tela da Dica',False,2);
end;
...
begin
  inherited;
  MostrarHintCampos('pela Dica','Dica',False);
end;
