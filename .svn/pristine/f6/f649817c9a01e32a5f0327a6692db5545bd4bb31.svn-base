program Corretor;

uses
  Forms,
  unPrincipal in 'unPrincipal.pas' {Form1},
  unCadastroBasico in 'Basicas\unCadastroBasico.pas' {frmCadastroBasico},
  unCadEstado in 'unCadEstado.pas' {frmCadEstado},
  unObjEstado in 'Objetos\unObjEstado.pas',
  unFuncoes in 'Classes\unFuncoes.pas',
  unDM in 'unDM.pas' {DM: TDataModule},
  unConstantes in 'Classes\unConstantes.pas',
  unObjCidade in 'Objetos\unObjCidade.pas',
  unCadCidade in 'unCadCidade.pas' {frmCadCidade},
  unSelecionaRegistro in 'Basicas\unSelecionaRegistro.pas' {frmSelecionaRegistro},
  unSelecionaRegistroEstado in 'unSelecionaRegistroEstado.pas' {frmSelecionaRegistroEstado},
  unPesquisandoRegistro in 'Basicas\unPesquisandoRegistro.pas' {frmPesquisandoRegistro},
  unPesquisandoCidade in 'unPesquisandoCidade.pas' {frmPesquisandoCidade},
  unLixo in 'unLixo.pas' {Form2},
  unCadBairro in 'unCadBairro.pas' {frmCadBairro},
  unObjBairro in 'Objetos\unObjBairro.pas',
  unPesquisandoBairro in 'unPesquisandoBairro.pas' {frmPesquisandoBairro},
  unSelecionaRegistroCidade in 'unSelecionaRegistroCidade.pas' {frmSelecionaRegistroCidade},
  unCadRua in 'unCadRua.pas' {frmCadRua},
  unObjRua in 'Objetos\unObjRua.pas',
  unPesquisandoRua in 'unPesquisandoRua.pas' {frmPesquisandoRua},
  unSelecionaRegistroBairro in 'unSelecionaRegistroBairro.pas' {frmSelecionaRegistroBairro},
  unObjConstrutora in 'Objetos\unObjConstrutora.pas',
  unCadConstrutora in 'unCadConstrutora.pas' {frmCadConstrutora},
  unPesquisandoConstrutora in 'unPesquisandoConstrutora.pas' {frmPesquisandoConstrutora},
  unCadProfissao in 'unCadProfissao.pas' {frmCadProfissao},
  unObjProfissao in 'Objetos\unObjProfissao.pas',
  unPesquisandoProfissao in 'unPesquisandoProfissao.pas' {frmPesquisandoProfissao},
  unCadCliente in 'unCadCliente.pas' {frmCadCliente},
  unObjCliente in 'Objetos\unObjCliente.pas',
  unPesquisandoCliente in 'unPesquisandoCliente.pas' {frmPesquisandoCliente},
  unSelecionaRegistroRua in 'unSelecionaRegistroRua.pas' {frmSelecionaRegistroRua},
  unSelecionaRegistroProfissao in 'unSelecionaRegistroProfissao.pas' {frmSelecionaRegistroProfissao},
  unCadTipoImovel in 'unCadTipoImovel.pas' {frmCadTipoImovel},
  unObjTipoImovel in 'Objetos\unObjTipoImovel.pas',
  unPesquisandoTipoImovel in 'unPesquisandoTipoImovel.pas' {frmPesquisandoTipoImovel},
  unCadPessoaFisica in 'unCadPessoaFisica.pas' {frmCadPessoaFisica},
  unObjPessoaFisica in 'Objetos\unObjPessoaFisica.pas',
  unPesquisandoPessoaFisica in 'unPesquisandoPessoaFisica.pas' {frmPesquisandoPessoaFisica},
  unCadAgenciamento in 'unCadAgenciamento.pas' {frmCadAgenciamento},
  unObjAgenciamento in 'Objetos\unObjAgenciamento.pas',
  unPesquisandoAgenciamento in 'unPesquisandoAgenciamento.pas' {frmPesquisandoAgenciamento},
  unObjImovel in 'Objetos\unObjImovel.pas',
  unCadImoveis in 'unCadImoveis.pas' {frmCadImoveis},
  unSelecionaRegistroTipoImovel in 'unSelecionaRegistroTipoImovel.pas' {frmSelecionaRegistroTipoImovel},
  unPesquisandoImovel in 'unPesquisandoImovel.pas' {frmPesquisandoImovel},
  unSistema in 'Classes\unSistema.pas',
  unCadAgenda in 'unCadAgenda.pas' {frmCadAgenda},
  unConsultaCompromissosAgenda in 'unConsultaCompromissosAgenda.pas' {frmConsultaCompromissosAgenda},
  unObjLog in 'Objetos\unObjLog.pas',
  unUsuario in 'Classes\unUsuario.pas',
  unLogin in 'unLogin.pas' {frmLogin},
  Unit3 in 'Unit3.pas' {Form3},
  unSelecionaRegistroImovel in 'unSelecionaRegistroImovel.pas' {frmSelecionaRegistroImovel},
  unVariaveis in 'Classes\unVariaveis.pas',
  unSelecionaRegistroPessoaFisica in 'unSelecionaRegistroPessoaFisica.pas' {frmSelecionaRegistroPessoaFisica},
  unSobre in 'unSobre.pas' {frmSobre},
  unCadMeioCaptacao in 'unCadMeioCaptacao.pas' {frmCadMeioCaptacao},
  unObjMeioCaptacao in 'Objetos\unObjMeioCaptacao.pas',
  unPesquisandoMeioCaptacao in 'unPesquisandoMeioCaptacao.pas' {frmPesquisandoMeioCaptacao},
  unAlterarLoginSistema in 'unAlterarLoginSistema.pas' {frmAlterarLoginSistema},
  unSelecionaRegistroMeioCaptacao in 'unSelecionaRegistroMeioCaptacao.pas' {frmSelecionaRegistroMeioCaptacao},
  unObjPessoaJuridica in 'Objetos\unObjPessoaJuridica.pas',
  unCadPessoaJuridica in 'unCadPessoaJuridica.pas' {frmCadPessoaJuridica},
  unPesquisandoPessoaJuridica in 'unPesquisandoPessoaJuridica.pas' {frmPesquisandoPessoaJuridica},
  unSelecionaRegistroPessoaJuridica in 'unSelecionaRegistroPessoaJuridica.pas' {frmSelecionaRegistroPessoaJuridica},
  unCadImovelInteresse in 'unCadImovelInteresse.pas' {frmCadImovelInteresse},
  unObjImovelInteresse in 'Objetos\unObjImovelInteresse.pas',
  unSelecionaRegistroCliente in 'unSelecionaRegistroCliente.pas' {frmSelecionaRegistroCliente},
  unObjVenda in 'Objetos\unObjVenda.pas',
  unCadVenda in 'unCadVenda.pas' {frmCadVenda},
  unSelecionaRegistroConstrutora in 'unSelecionaRegistroConstrutora.pas' {frmSelecionaRegistroConstrutora},
  unSelecionaRegistroAgenciamento in 'unSelecionaRegistroAgenciamento.pas' {frmSelecionaRegistroAgenciamento},
  unExibeDados in 'Basicas\unExibeDados.pas' {frmExibeDados},
  unExibeDadosAgenciamento in 'unExibeDadosAgenciamento.pas' {frmExibeDadosAgenciamento},
  unPesquisa in 'Basicas\unPesquisa.pas' {frmPesquisar},
  unPesquisarAgenciamentos in 'unPesquisarAgenciamentos.pas' {frmPesquisarAgenciamentos},
  unPesquisarImoveisInteresse in 'unPesquisarImoveisInteresse.pas' {frmPesquisarImoveisInteresse},
  unPesquisarVendas in 'unPesquisarVendas.pas' {frmPesquisarVendas},
  unConjuntoDados in 'Basicas\unConjuntoDados.pas',
  unConjuntoDadosCidade in 'Conjunto de Dados\unConjuntoDadosCidade.pas',
  unConjuntoDadosEstado in 'Conjunto de Dados\unConjuntoDadosEstado.pas',
  unConjuntoDadosBairro in 'Conjunto de Dados\unConjuntoDadosBairro.pas',
  unCadClientePessoaJuridica in 'unCadClientePessoaJuridica.pas' {frmCadClientePessoaJuridica},
  unObjClientePessoaJuridica in 'Objetos\unObjClientePessoaJuridica.pas',
  unPesquisandoClientePessoaJuridica in 'unPesquisandoClientePessoaJuridica.pas' {frmPesquisandoClientePessoaJuridica},
  unSelecionaRegistroClientePessoaJuridica in 'unSelecionaRegistroClientePessoaJuridica.pas' {frmSelecionaRegistroClientePessoaJuridica},
  unConjuntoDadosTipoImovel in 'Conjunto de Dados\unConjuntoDadosTipoImovel.pas',
  unConjuntoDadosMeioCaptacao in 'Conjunto de Dados\unConjuntoDadosMeioCaptacao.pas',
  unConjuntoDadosProfissao in 'Conjunto de Dados\unConjuntoDadosProfissao.pas',
  unConjuntoDadosRua in 'Conjunto de Dados\unConjuntoDadosRua.pas',
  unConjuntoDadosAgencPessoaFisica in 'Conjunto de Dados\unConjuntoDadosAgencPessoaFisica.pas',
  unConjuntoDadosAgencPessoaJuridica in 'Conjunto de Dados\unConjuntoDadosAgencPessoaJuridica.pas',
  unConjuntoDadosClientePessoaFisica in 'Conjunto de Dados\unConjuntoDadosClientePessoaFisica.pas',
  unConjuntoDadosClientePessoaJuridica in 'Conjunto de Dados\unConjuntoDadosClientePessoaJuridica.pas',
  unConjuntoDadosConstrutora in 'Conjunto de Dados\unConjuntoDadosConstrutora.pas',
  unDicas in 'Basicas\unDicas.pas' {frmDicas},
  unCadGrupoConfiguracaoImovel in 'unCadGrupoConfiguracaoImovel.pas' {frmCadGrupoConfiguracaoImovel},
  unObjGrupoConfiguracaoImovel in 'Objetos\unObjGrupoConfiguracaoImovel.pas',
  unPesquisandoGrupoConfiguracaoImovel in 'unPesquisandoGrupoConfiguracaoImovel.pas' {frmPesquisandoGrupoConfiguracaoImovel},
  unSelecionaRegistroGrupoConfiguracaoImovel in 'unSelecionaRegistroGrupoConfiguracaoImovel.pas' {frmSelecionaRegistroGrupoConfiguracaoImovel},
  unCadSubGrupoConfiguracaoImovel in 'unCadSubGrupoConfiguracaoImovel.pas' {frmCadSubGrupoConfiguracaoImovel},
  unObjSubGrupoConfImovel in 'Objetos\unObjSubGrupoConfImovel.pas',
  unConjuntoDadosGrupo in 'Conjunto de Dados\unConjuntoDadosGrupo.pas',
  unPesquisandoSubGrupoConfImovel in 'unPesquisandoSubGrupoConfImovel.pas' {frmPesquisandoSubGrupoConfImovel},
  unSelecionaRegistroSubGrupoConfImovel in 'unSelecionaRegistroSubGrupoConfImovel.pas' {frmSelecionaRegistroSubGrupoConfiguracaoImovel},
  unCadDicas in 'unCadDicas.pas' {frmCadDica},
  unObjDica in 'Objetos\unObjDica.pas',
  unPesquisandoDica in 'unPesquisandoDica.pas' {frmPesquisandoDica},
  unSelecionarCaracImovel in 'unSelecionarCaracImovel.pas' {frmSelecionarCaracImovel},
  unCadGrupoBemNegociavel in 'unCadGrupoBemNegociavel.pas' {frmCadGrupoBemNegociavel},
  unObjGrupoBemNegociavel in 'Objetos\unObjGrupoBemNegociavel.pas',
  unPesquisandoGrupoBemNegociavel in 'unPesquisandoGrupoBemNegociavel.pas' {frmPesquisandoGrupoBemNegociavel},
  unSelecionaRegistroGrupoBemNegociavel in 'unSelecionaRegistroGrupoBemNegociavel.pas' {frmSelecionaRegistroGrupoBemNegociavel},
  unCadSubGrupoBemNegociavel in 'unCadSubGrupoBemNegociavel.pas' {frmCadSubGrupoBemNegociavel},
  unObjSubGrupoBemNegociavel in 'Objetos\unObjSubGrupoBemNegociavel.pas',
  unConjuntoDadosGrupoBemNegociavel in 'Conjunto de Dados\unConjuntoDadosGrupoBemNegociavel.pas',
  unPesquisandoSubGrupoBemNegociavel in 'unPesquisandoSubGrupoBemNegociavel.pas' {frmPesquisandoSubGrupoBemNegociavel},
  unSelecionaRegistroSubGrupoBemNegociavel in 'unSelecionaRegistroSubGrupoBemNegociavel.pas' {frmSelecionaRegistroSubGrupoBemNegociavel},
  unSelecionarCaracBemNegociavel in 'unSelecionarCaracBemNegociavel.pas' {frmSelecionarCaracBemNegociavel},
  unBuscarAgenciamentosParaImovelInteresse in 'unBuscarAgenciamentosParaImovelInteresse.pas',
  unCadRegiaoLocalidade in 'unCadRegiaoLocalidade.pas' {frmCadRegiaoLocalidade},
  unObjRegiaoLocalidade in 'Objetos\unObjRegiaoLocalidade.pas',
  unPesquisandoRegiaoLocalidade in 'unPesquisandoRegiaoLocalidade.pas' {frmPesquisandoRegiaoLocalidade},
  unSelecionaRegistroRegiaoLocalidade in 'unSelecionaRegistroRegiaoLocalidade.pas' {frmSelecionaRegistroRegiaoLocalidade},
  unConjuntoDadosRegiaoLocalidade in 'Conjunto de Dados\unConjuntoDadosRegiaoLocalidade.pas',
  unExibirAgenciamentosParaImovelInteresse in 'unExibirAgenciamentosParaImovelInteresse.pas' {frmAgenciamentosParaImovelInteresse},
  unExplicacao in 'unExplicacao.pas' {frmExplicacao},
  unBuscarImovelInteresseParaAgenciamento in 'unBuscarImovelInteresseParaAgenciamento.pas',
  unExibirImoveisInteresseParaAgenciamento in 'unExibirImoveisInteresseParaAgenciamento.pas' {frmImoveisInteresseParaAgenciamento},
  unExibeDadosImoveisInteresse in 'unExibeDadosImoveisInteresse.pas' {frmExibeDadosImoveisInteresse},
  unRegistrarSistema in 'unRegistrarSistema.pas' {frmRegistroSistema},
  unBckupRestore in 'unBckupRestore.pas',
  unRestaurarBackup in 'unRestaurarBackup.pas' {frmRestaurarBackup},
  unExibeDadosVenda in 'unExibeDadosVenda.pas' {frmExibeDadosVenda};

{$R *.res}

begin
  Application.Initialize;
  Application.Title := 'GICI';
  Application.CreateForm(TDM, DM);
  Application.CreateForm(TfrmPrincipal, frmPrincipal);
  Application.Run;
end.
