...
function TfrmPesquisandoBairro.GetNomeCidade(pnCodCidade:Integer): String;
begin
  zQryCidade.Close;
  zQryCidade.SQL.Clear;
  zQryCidade.SQL.Add('SELECT * FROM cidade WHERE codigo=:codigo');
  zQryCidade.ParamByName('codigo').AsInteger := pnCodCidade;
  zQryCidade.Open;
  if(zQryCidade.Active)and(zQryCidade.RecordCount=1)then
    Result := zQryCidadeNOME.AsString
  else
