...
uses unCadRua, unConstantes, unCadCliente, unCadPessoaFisica, unCadImoveis,
  ...
  unCadRegiaoLocalidade, unPesquisarAgenciamentos, unPesquisarVendas;
...
begin
  ...
  begin
    ...
    else if(Self.FiTelaChamou = FORM_CAD_REGIAO_LOCALIDADE)then //TfrmCadRegiaoLocalidade
    begin
      frmCadRegiaoLocalidade.FoBairro.codigo := FiCodigo;
      frmCadRegiaoLocalidade.FoBairro.nome := FsNome;
    end
    else if(Self.FiTelaChamou = FORM_PESQUISAR_AGENCIAMENTO)then //TfrmPesquisarAgenciamentos
    begin
      frmPesquisarAgenciamentos.FoBairro.codigo := FiCodigo;
      frmPesquisarAgenciamentos.FoBairro.nome := FsNome;
    end
    else if(Self.FiTelaChamou = FORM_PESQUISAR_VENDA)then //TfrmPesquisarVendas
    begin
      frmPesquisarVendas.FoBairro.codigo := FiCodigo;
      frmPesquisarVendas.FoBairro.nome := FsNome;
    end;
  end;
...
begin
  ...
  else if(Self.FiTelaChamou = FORM_CAD_PESSOA_JURIDICA)then //TfrmCadRegiaoLocalidade
    frmCadRegiaoLocalidade.passarParametro(PRM_SELECIONAR_TODOS_REGISTROS_CONJUNTO_DADOS, vParametros)
...
begin
  ...
  begin
    if(Self.FiTelaChamou = FORM_CAD_CLIENTE)then //TfrmCadCliente
      ...
      frmCadRegiaoLocalidade.passarParametro(PRM_NAO_LIMPAR_ENDERECO, null)

    else if(Self.FiTelaChamou = FORM_PESQUISAR_AGENCIAMENTO)then //TfrmPesquisarAgenciamentos
      frmPesquisarAgenciamentos.passarParametro(PRM_NAO_LIMPAR_ENDERECO, null)

    else if(Self.FiTelaChamou = FORM_PESQUISAR_VENDA)then //TfrmPesquisarVendas
      frmPesquisarVendas.passarParametro(PRM_NAO_LIMPAR_ENDERECO, null);
  end;

  inherited;
end;

procedure TfrmSelecionaRegistroBairro.FormShow(Sender: TObject);
begin
  inherited;
  FoFuncoes.focarComponente(plsEdValorPesquisar);
end;
