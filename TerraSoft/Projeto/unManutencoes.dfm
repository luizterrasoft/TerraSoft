object frmManutencoes: TfrmManutencoes
  Left = 459
  Top = 75
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'MANUTEN'#199#213'ES/DESPESAS'
  ClientHeight = 554
  ClientWidth = 432
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PnTopo: TPanel
    Left = 0
    Top = 0
    Width = 432
    Height = 46
    Align = alTop
    Color = clMoneyGreen
    TabOrder = 0
    object lblTitulo: TLabel
      Left = 38
      Top = 11
      Width = 356
      Height = 24
      Caption = 'REGISTRO DE MANUT./DESPESAS'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -19
      Font.Name = 'MS Reference Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object PnDados: TPanel
    Left = 0
    Top = 46
    Width = 432
    Height = 468
    Align = alClient
    TabOrder = 1
    object Bevel1: TBevel
      Left = 8
      Top = 274
      Width = 410
      Height = 2
    end
    object lblProdutos: TLabel
      Left = 115
      Top = 277
      Width = 205
      Height = 20
      Caption = 'PRODUTOS/SERVI'#199'OS'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -16
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
    end
    object lblEquipamento: TLabel
      Left = 13
      Top = 13
      Width = 5
      Height = 16
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object PnDadosProdutos: TPanel
      Left = 0
      Top = 299
      Width = 425
      Height = 180
      Enabled = False
      TabOrder = 1
      object Produto: TLabel
        Left = 8
        Top = 7
        Width = 78
        Height = 13
        Caption = 'Produto/Servi'#231'o'
      end
      object lblProduto: TLabel
        Left = 104
        Top = 23
        Width = 6
        Height = 20
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label5: TLabel
        Left = 8
        Top = 46
        Width = 26
        Height = 13
        Caption = 'Qtde.'
      end
      object Label10: TLabel
        Left = 72
        Top = 46
        Width = 24
        Height = 13
        Caption = 'Valor'
      end
      object lblTotal: TLabel
        Left = 248
        Top = 63
        Width = 45
        Height = 20
        Caption = '0,00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold, fsItalic]
        ParentFont = False
      end
      object Label12: TLabel
        Left = 144
        Top = 48
        Width = 62
        Height = 13
        Caption = 'Data Compra'
      end
      object edtCodProduto: TEdit
        Left = 8
        Top = 23
        Width = 57
        Height = 19
        CharCase = ecUpperCase
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 0
        OnExit = edtCodProdutoExit
      end
      object BtnBuscaProduto: TBitBtn
        Left = 72
        Top = 23
        Width = 25
        Height = 19
        TabOrder = 1
        OnClick = BtnBuscaProdutoClick
        Glyph.Data = {
          
          A89FDFC0C0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      end
      object edtQtde: TEdit
        Left = 8
        Top = 63
        Width = 57
        Height = 19
        CharCase = ecUpperCase
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 5
        Text = '1'
      end
      object edtCusto: TEdit
        Left = 72
        Top = 63
        Width = 65
        Height = 19
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 6
        Text = '0,00'
        OnExit = edtCustoExit
      end
      object ChBxBaixarEstoque: TCheckBox
        Left = 248
        Top = 46
        Width = 105
        Height = 16
        Caption = 'Baixar do Estoque'
        Checked = True
        State = cbChecked
        TabOrder = 3
      end
      object BtnIncluir: TBitBtn
        Left = 360
        Top = 46
        Width = 59
        Height = 21
        Caption = 'Incluir'
        TabOrder = 4
        OnClick = BtnIncluirClick
      end
      object BtnExcluir: TBitBtn
        Left = 360
        Top = 71
        Width = 59
        Height = 21
        Caption = 'Excluir'
        TabOrder = 8
        OnClick = BtnExcluirClick
      end
      object PnProdutos: TPanel
        Left = -2
        Top = 98
        Width = 427
        Height = 103
        TabOrder = 9
        object GrdProdutos: TDBGrid
          Left = 1
          Top = 1
          Width = 425
          Height = 101
          Align = alClient
          Color = 13565951
          DataSource = dsProdutos
          Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
          TabOrder = 0
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'MS Sans Serif'
          TitleFont.Style = []
          OnCellClick = GrdProdutosCellClick
          OnKeyDown = GrdProdutosKeyDown
          OnKeyPress = GrdProdutosKeyPress
          OnKeyUp = GrdProdutosKeyUp
          Columns = <
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'CODPRODUTO'
              Title.Alignment = taCenter
              Title.Caption = 'C'#243'd. Prod.'
              Width = 68
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'DESCRICAO'
              Title.Alignment = taCenter
              Title.Caption = 'Descri'#231#227'o'
              Width = 235
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'QTDE'
              Title.Alignment = taCenter
              Title.Caption = 'Qtde.'
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'VALORCUSTO'
              Title.Alignment = taCenter
              Title.Caption = 'Unit'#225'rio'
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'TOTAL'
              Title.Alignment = taCenter
              Title.Caption = 'Total'
              Visible = True
            end
            item
              Alignment = taCenter
              Expanded = False
              FieldName = 'BAIXAESTOQUE'
              Title.Alignment = taCenter
              Title.Caption = 'Bx. Estoque'
              Visible = False
            end>
        end
      end
      object edtProduto: TEdit
        Left = 104
        Top = 23
        Width = 305
        Height = 19
        CharCase = ecUpperCase
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 2
      end
      object mskDataCompra: TMaskEdit
        Left = 144
        Top = 64
        Width = 73
        Height = 19
        Color = 13565951
        Ctl3D = False
        EditMask = '99/99/9999;1;_'
        MaxLength = 10
        ParentCtl3D = False
        TabOrder = 7
        Text = '  /  /    '
      end
    end
    object PnDadosManutencao: TPanel
      Left = 0
      Top = 39
      Width = 425
      Height = 235
      TabOrder = 0
      object Label1: TLabel
        Left = 10
        Top = 3
        Width = 33
        Height = 13
        Caption = 'C'#243'digo'
      end
      object Label2: TLabel
        Left = 81
        Top = 3
        Width = 48
        Height = 13
        Caption = 'Descri'#231#227'o'
      end
      object Label3: TLabel
        Left = 10
        Top = 37
        Width = 54
        Height = 13
        Caption = 'Fornecedor'
      end
      object lblFornecedor: TLabel
        Left = 114
        Top = 50
        Width = 5
        Height = 16
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label6: TLabel
        Left = 10
        Top = 71
        Width = 83
        Height = 13
        Caption = 'C'#243'd. Plano Conta'
      end
      object lblPlanoConta: TLabel
        Left = 114
        Top = 82
        Width = 5
        Height = 16
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label8: TLabel
        Left = 10
        Top = 171
        Width = 25
        Height = 13
        Caption = 'Obs.:'
      end
      object Label4: TLabel
        Left = 82
        Top = 135
        Width = 104
        Height = 13
        Caption = 'Valor da M'#227'o de Obra'
        Visible = False
      end
      object lblTotalManutencao: TLabel
        Left = 355
        Top = 98
        Width = 45
        Height = 20
        Alignment = taRightJustify
        Caption = '0,00'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clRed
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold, fsItalic]
        ParentFont = False
      end
      object Label7: TLabel
        Left = 352
        Top = 73
        Width = 55
        Height = 20
        Caption = 'Total:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -16
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold, fsItalic]
        ParentFont = False
      end
      object Label9: TLabel
        Left = 10
        Top = 104
        Width = 120
        Height = 13
        Caption = 'C'#243'd. Sub-Plano de Conta'
      end
      object lblSubPlano: TLabel
        Left = 140
        Top = 115
        Width = 5
        Height = 16
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object Label11: TLabel
        Left = 338
        Top = 3
        Width = 21
        Height = 13
        Caption = 'Tipo'
      end
      object lblNF: TLabel
        Left = 11
        Top = 136
        Width = 53
        Height = 13
        Caption = 'Nota Fiscal'
      end
      object edtCodigo: TEdit
        Left = 10
        Top = 19
        Width = 65
        Height = 19
        CharCase = ecUpperCase
        Color = clMoneyGreen
        Ctl3D = False
        Enabled = False
        ParentCtl3D = False
        TabOrder = 0
      end
      object edtDescricao: TEdit
        Left = 81
        Top = 19
        Width = 251
        Height = 19
        CharCase = ecUpperCase
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 1
      end
      object edtCodPlanoConta: TEdit
        Left = 10
        Top = 86
        Width = 65
        Height = 19
        CharCase = ecUpperCase
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 5
        OnEnter = edtCodPlanoContaEnter
        OnExit = edtCodPlanoContaExit
      end
      object edtCodFornecedor: TEdit
        Left = 10
        Top = 52
        Width = 65
        Height = 19
        CharCase = ecUpperCase
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 4
        OnEnter = edtCodFornecedorEnter
        OnExit = edtCodFornecedorExit
      end
      object RchEdtObs: TRichEdit
        Left = 10
        Top = 184
        Width = 392
        Height = 44
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 13
      end
      object BtnBuscaFornecedor: TBitBtn
        Left = 81
        Top = 51
        Width = 25
        Height = 19
        TabOrder = 3
        OnClick = BtnBuscaFornecedorClick
        Glyph.Data = {
          
          A89FDFC0C0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      end
      object BtnBuscaPlanoConta: TBitBtn
        Left = 81
        Top = 86
        Width = 25
        Height = 19
        TabOrder = 6
        OnClick = BtnBuscaPlanoContaClick
        Glyph.Data = {
          
          A89FDFC0C0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      end
      object BtnGravar: TBitBtn
        Left = 248
        Top = 136
        Width = 75
        Height = 33
        Caption = 'GRAVAR'
        TabOrder = 9
        OnClick = BtnGravarClick
      end
      object edtValorServico: TEdit
        Left = 82
        Top = 150
        Width = 71
        Height = 19
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 12
        Text = '0,00'
        Visible = False
        OnExit = edtValorServicoExit
      end
      object BtnFaturar: TBitBtn
        Left = 328
        Top = 136
        Width = 75
        Height = 33
        Caption = 'FATURAR'
        TabOrder = 10
        OnClick = BtnFaturarClick
      end
      object edtCodSubPlano: TEdit
        Left = 10
        Top = 117
        Width = 62
        Height = 19
        CharCase = ecUpperCase
        Color = 13565951
        Ctl3D = False
        ParentCtl3D = False
        TabOrder = 7
        OnExit = edtCodSubPlanoExit
      end
      object BtnBuscaSubPlano: TBitBtn
        Left = 81
        Top = 117
        Width = 25
        Height = 19
        TabOrder = 8
        OnClick = BtnBuscaSubPlanoClick
        Glyph.Data = {
          
          A89FDFC0C0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
      end
      object CbxTipo: TComboBox
        Left = 338
        Top = 20
        Width = 79
        Height = 21
        Style = csDropDownList
        Color = 13565951
        ItemHeight = 13
        ItemIndex = 1
        TabOrder = 2
        Text = 'GERAL'
        Items.Strings = (
          ''
          'GERAL'
          'ABASTECIMENTO'
          'TROCA DE '#211'LEO'
          'LAVA'#199#195'O')
      end
      object edtNotaFiscal: TEdit
        Left = 11
        Top = 150
        Width = 62
        Height = 19
        CharCase = ecUpperCase
        Color = 13565951
        Ctl3D = False
        MaxLength = 10
        ParentCtl3D = False
        TabOrder = 11
      end
    end
  end
  object PnRodape: TPanel
    Left = 0
    Top = 514
    Width = 432
    Height = 40
    Align = alBottom
    Color = clMoneyGreen
    TabOrder = 2
    object BtnConfirmar: TBitBtn
      Left = 114
      Top = 6
      Width = 97
      Height = 29
      Caption = 'CONCLUIR - F5'
      TabOrder = 0
      OnClick = BtnConfirmarClick
    end
    object BtnCancelar: TBitBtn
      Left = 218
      Top = 6
      Width = 97
      Height = 29
      Caption = 'CANCELAR - ESC'
      TabOrder = 1
      OnClick = BtnCancelarClick
    end
  end
  object dsManutencoes: TDataSource
    DataSet = cdsManutencoes
    Left = 176
    Top = 89
  end
  object cdsManutencoes: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspManutencoes'
    Left = 208
    Top = 89
  end
  object dspManutencoes: TDataSetProvider
    DataSet = qryManutencoes
    Left = 240
    Top = 89
  end
  object qryManutencoes: TSQLQuery
    DataSource = dsManutencoes
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.Con
    Left = 272
    Top = 89
  end
  object dsProdutos: TDataSource
    DataSet = cdsProdutos
    Left = 176
    Top = 118
  end
  object dspProdutos: TDataSetProvider
    DataSet = qryProdutos
    Left = 240
    Top = 118
  end
  object cdsProdutos: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspProdutos'
    Left = 208
    Top = 118
  end
  object qryProdutos: TSQLQuery
    DataSource = dsProdutos
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.Con
    Left = 272
    Top = 118
  end
  object dsAux: TDataSource
    DataSet = cdsAux
    Left = 176
    Top = 150
  end
  object cdsAux: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspAux'
    Left = 208
    Top = 150
  end
  object dspAux: TDataSetProvider
    DataSet = qryAux
    Left = 240
    Top = 150
  end
  object qryAux: TSQLQuery
    DataSource = dsAux
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.Con
    Left = 272
    Top = 150
  end
end
