object frmPermissoesUsuarios: TfrmPermissoesUsuarios
  Left = 373
  Top = 224
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'PERMISS'#213'ES USU'#193'RIO'
  ClientHeight = 355
  ClientWidth = 509
  Color = clMoneyGreen
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poScreenCenter
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnKeyDown = FormKeyDown
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Bevel1: TBevel
    Left = 7
    Top = 83
    Width = 490
    Height = 2
  end
  object Label5: TLabel
    Left = 193
    Top = 56
    Width = 105
    Height = 24
    Caption = 'Bloqueios'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -19
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold, fsItalic]
    ParentFont = False
  end
  object Label6: TLabel
    Left = 16
    Top = 88
    Width = 75
    Height = 16
    Caption = 'Bloquear:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold, fsItalic]
    ParentFont = False
  end
  object Label1: TLabel
    Left = 8
    Top = 8
    Width = 73
    Height = 16
    Caption = 'USU'#193'RIO:'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblUsuario: TLabel
    Left = 88
    Top = 8
    Width = 5
    Height = 16
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object lblSair: TLabel
    Left = 416
    Top = 8
    Width = 79
    Height = 16
    Caption = 'ESC - SAIR'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object PnBloqueios: TPanel
    Left = 8
    Top = 109
    Width = 489
    Height = 236
    TabOrder = 0
    object lblForm: TLabel
      Left = 8
      Top = 8
      Width = 21
      Height = 13
      Caption = 'Tela'
    end
    object lblTela: TLabel
      Left = 96
      Top = 24
      Width = 5
      Height = 16
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object Label7: TLabel
      Left = 8
      Top = 48
      Width = 65
      Height = 13
      Caption = 'Op'#231#227'o/Bot'#227'o'
    end
    object lblOpcaoBotao: TLabel
      Left = 96
      Top = 64
      Width = 5
      Height = 16
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object edtCodTela: TEdit
      Left = 8
      Top = 24
      Width = 49
      Height = 19
      CharCase = ecUpperCase
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 0
      OnEnter = edtCodTelaEnter
      OnExit = edtCodTelaExit
    end
    object BtnBuscaTela: TBitBtn
      Left = 64
      Top = 24
      Width = 25
      Height = 19
      TabOrder = 1
      OnClick = BtnBuscaTelaClick
      Glyph.Data = {
        
        A89FDFC0C0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object edtCodOpcaoBotao: TEdit
      Left = 8
      Top = 64
      Width = 49
      Height = 19
      Ctl3D = False
      ParentCtl3D = False
      TabOrder = 2
      OnEnter = edtCodOpcaoBotaoEnter
      OnExit = edtCodOpcaoBotaoExit
    end
    object BtnBuscaOpcaoBotao: TBitBtn
      Left = 64
      Top = 64
      Width = 25
      Height = 21
      TabOrder = 3
      OnClick = BtnBuscaOpcaoBotaoClick
      Glyph.Data = {
        
        A89FDFC0C0FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    end
    object BtnBloquear: TBitBtn
      Left = 400
      Top = 16
      Width = 75
      Height = 25
      Caption = 'Bloquear'
      TabOrder = 4
      OnClick = BtnBloquearClick
    end
    object BtnDesbloquear: TBitBtn
      Left = 400
      Top = 48
      Width = 75
      Height = 25
      Caption = 'Desbloquear'
      TabOrder = 5
      OnClick = BtnDesbloquearClick
    end
    object GrdPermissoes: TDBGrid
      Left = 8
      Top = 88
      Width = 473
      Height = 145
      DataSource = dsPermissoes
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 6
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnCellClick = GrdPermissoesCellClick
      OnTitleClick = GrdPermissoesTitleClick
      Columns = <
        item
          Expanded = False
          FieldName = 'FORM'
          Title.Alignment = taCenter
          Title.Caption = 'Tela'
          Width = 224
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'BOTAO'
          Title.Alignment = taCenter
          Title.Caption = 'Bot'#227'o/Fun'#231#227'o'
          Width = 217
          Visible = True
        end>
    end
  end
  object dsPermissoes: TDataSource
    DataSet = cdsPermissoes
    Left = 248
    Top = 8
  end
  object cdsPermissoes: TClientDataSet
    Aggregates = <>
    Params = <>
    ProviderName = 'dspPermissoes'
    Left = 280
    Top = 8
  end
  object dspPermissoes: TDataSetProvider
    DataSet = qryPermissoes
    Left = 312
    Top = 8
  end
  object qryPermissoes: TSQLQuery
    DataSource = dsPermissoes
    MaxBlobSize = -1
    Params = <>
    SQLConnection = DM.Con
    Left = 344
    Top = 8
  end
end
