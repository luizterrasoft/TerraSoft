...
type
  ...
  private
    procedure GravaBackup; // grava o backup no banco de dados
...
uses IBDatabase, Principal, unDM, unPrincipal;
...
begin
  try
    ...
    begin
      DM.Con.Connected:=False;//desconecta da base de dados
      DatabaseName:=frmBackup.EBase.Text;//caminho da base de dados
    ...
    GravaBackup; // GRAVA O BACKUP NO BANCO DE DADOS
  except
    on E: Exception do
    begin
...
procedure TFGerarBackup.GravaBackup;
begin
  try
    qryBackup.Close;
    qryBackup.SQL.Clear;
    qryBackup.SQL.Add('INSERT INTO backups');
    qryBackup.SQL.Add('(data,destino,hora');

    if frmPrincipal.iCodUsuario > 0 then
      qryBackup.SQL.Add(',codusuario');

    qryBackup.SQL.Add(')');

    qryBackup.SQL.Add('VALUES (:dt,:dest,:hr');

    if frmPrincipal.iCodUsuario > 0 then
      qryBackup.SQL.Add(',:usu');

    qryBackup.SQL.Add(')');

    qryBackup.ParamByName('dt').AsDate := Date;

    if frmPrincipal.iCodUsuario > 0 then
      qryBackup.ParamByName('usu').AsInteger   := frmPrincipal.iCodUsuario;
      
    qryBackup.ParamByName('dest').AsString   := SDArquBack.FileName;
    qryBackup.ParamByName('hr').AsTime       := Time;
    qryBackup.ExecSQL();
  except
    MessageDlg('Ocorreu um erro ao gravar o backup no banco de dados!',mtError,[mbOk],0);
  end;
end;
