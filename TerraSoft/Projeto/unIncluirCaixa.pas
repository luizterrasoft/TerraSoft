...
begin
  if (edtDescricao.Text = trim('')) then
    begin
      MessageDlg('Por favor digite uma descrição para o lançamento!',mtWarning,[mbOk],0);
  ...
  if (mskData.Text = '  /  /    ') then
    begin
      MessageDlg('Por favor digite uma data válida!',mtWarning,[mbOk],0);
      mskData.SetFocus;
      Exit;
    end;
  ...
  if (CbxTipo.ItemIndex < 0) then
    begin
      MessageDlg('Por favor selecione o tipo do lançamento!',mtWarning,[mbOk],0);
      CbxTipo.SetFocus;
      Exit;
    end;
  ...
  try
    with qryCaixa, SQL do
      begin
        ...
        Add('VALUES (:con,:vlr,:desc,:dt,:tip)');
    ...
    MessageDlg('Erro ao incluir o lançamento no caixa!',mtWarning,[mbOk],0);
  end;
